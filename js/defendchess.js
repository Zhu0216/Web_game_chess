...
function sendScore(name, score) {
    ...
    .catch(err => console.error('傳送分數錯誤:', err));
}
...
function loadScores() {
    fetch('scores.php')
        .then(res => res.json())
        .then(data => {
            let table = document.getElementById('allscore-table');
            while (table.rows.length > 1) table.deleteRow(1);
            data.forEach((row, index) => {
                let tr = table.insertRow();
                tr.insertCell(0).textContent = row.name;
                tr.insertCell(1).textContent = row.score;
                tr.insertCell(2).textContent = index + 1;
                tr.insertCell(3).textContent = row.created_at;
            });
        });
}
...
function addenemy(type) {
    ...
    var randomInt = Math.floor(Math.random() * (max - min)) + min;
    var AImg = document.createElement("img");
    AImg.src = "./image/chess/black-" + type + ".png";
    AImg.classList.add("chess");
    AImg.id = "black-" + type + num;
    AImg.onclick = function () {
        childclick(AImg);
    }
...
function addallies(type) {
    var min = 0;
    var max = 8;
    var randomInt = Math.floor(Math.random() * (max - min)) + min;
    var AImg = document.createElement("img");
    AImg.src = "./image/chess/white-" + type + ".png";
    AImg.classList.add("chess");
    if (type == "king") {
        AImg.id = "white-" + type;
    } else {
        AImg.id = "white-" + type + num;
    }
    AImg.onclick = function () {
        childclick(AImg);
    }
    if (type == "pawn") {
        var randomgrid = document.getElementById("grid-cell-6-" + randomInt);
    } else {
        var randomgrid = document.getElementById("grid-cell-7-" + randomInt);
    }
    if (randomgrid.hasChildNodes()) {
        addallies(type);
    } else {
        ...
        for (var j = 7; j >= 0; j--) {
            ...
            if (grid.hasChildNodes()) {
                ...
                if (grid.firstElementChild.id.substring(0, 8) == "obstacle") {
                    ...
                    if (newgrid2.hasChildNodes() && newgrid2.firstElementChild.id.substring(0, 8) == "pentacle") {
                        newgrid2.removeChild(newgrid2.firstChild);
                        pentaclearray.push(gridElementId);
                    } else {
...
function clearScores() {
    if (!confirm("確定要清除所有分數嗎？")) return;

    fetch('clear_scores.php')
        .then(res => res.text())
        .then(msg => {
            alert(msg);
            loadScores(); // 清除後重新載入排行榜
        })
        .catch(err => console.error("清除失敗：", err));
}

function dialog(reason) {
    var dialog = document.createElement('div');
    dialog.style.whiteSpace = "pre-wrap"; // 支援換行
    dialog.textContent = "你輸了!!" + reason + "!!\n分數: " + score + "分";
    dialog.classList.add("dialog");
    document.body.appendChild(dialog);
}

function gameOver(reason) {
    let playerName = prompt("請輸入你的名稱（最多12字）", "Player") || "Player";

    // 限制最多 12 字元
    if (playerName.length > 12) {
        playerName = playerName.substring(0, 12);
        alert("名稱過長，已自動截斷為：" + playerName);
    }
